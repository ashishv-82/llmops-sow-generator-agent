...
from tests.evals.metrics import false_positive_rate, issue_detection_accuracy, score_accuracy
...
def test_compliance_review_accuracy(eval_cases):
    ...
    for case in eval_cases:
        ...
        mandatory_result = check_mandatory_clauses_v2.invoke(
            {"sow_text": sow_text, "product": product, "client_tier": client_tier}
        )

        prohibited_result = check_prohibited_terms.invoke({"sow_text": sow_text})
        ...
        if not sla_result.get("compliant", True):
            for issue in sla_result.get("issues", []):
                detected_issues.append(
                    {"category": "SLA", "severity": "HIGH", "description": issue}
                )
        ...
        test_passed = (
            accuracy_metrics["f1_score"] >= 0.7  # F1 score threshold
            and fp_rate <= 0.3  # Max 30% false positives
            and score_ok  # Score within expected range
        )
    ...
    # Print summary
    print(f"\n{'='*60}")
    print("Compliance Review Evaluation Summary")
    print(f"{'='*60}")
